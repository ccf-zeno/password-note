...
const NoteDetail = () => {
  ...
  // 获取当前笔记index和数据
  const noteIndex = useMemo(() => notes.findIndex(note => note.id === currentId), [notes, currentId]);
  const note: Note | undefined = noteIndex !== -1 ? notes[noteIndex] : undefined;
  ...
  const setInfoList = (infoList: Info[]) => {
    dispatch(updateNote({ id: currentId, description: note.description, info: infoList }));
  };
  ...
  const onModalOkPress = () => {
    ...
    if (editIndex === null) {
      if (!editInfo.value.trim()) {
        toast.show({ description: '信息内容不能为空' });
        return;
      }
      ...
      <ScrollView style={styles.page} keyboardShouldPersistTaps="handled">
        ...
        <Modal isOpen={visible} onClose={() => setVisible(false)}>
          <Modal.Content maxWidth="400px" p={4}>
            <Modal.CloseButton />
            <Modal.Header>{editIndex === null ? '新增信息' : '编辑信息'}</Modal.Header>
            <Modal.Body>
              <FormControl mb={3}>
            ...
            <Modal.Footer>
              <Button.Group space={2}>
                <Button variant="ghost" colorScheme="coolGray" onPress={() => setVisible(false)}>
                  取消
                </Button>
                <Button onPress={onModalOkPress}>确认</Button>
              </Button.Group>
            </Modal.Footer>
          </Modal.Content>
        </Modal>
