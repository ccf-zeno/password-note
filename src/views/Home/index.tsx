...
function HomeScreen() {
  ...
  // ─── 记录数文字淡出 ───
  const countTextAnimStyle = useAnimatedStyle(() => {
    'worklet';
    const p = searchProgress.value;
    return {
      opacity: interpolate(p, [0, 0.4], [1, 0], Extrapolation.CLAMP),
      transform: [
        {translateY: interpolate(p, [0, 0.5], [0, -15], Extrapolation.CLAMP)},
      ],
    };
  });
  ...
  const handleExport = useCallback(async () => {
    closeDrawer();
    try {
      const path = await exportNotes(notes);
    ...
    <View style={StyleSheet.absoluteFill}>
      ...
      <ReAnimated.View
        style={[
          ...
          {top: centerTop - 36, alignItems: 'center'},
          countTextAnimStyle,
        ]}
        pointerEvents={isActive ? 'none' : 'auto'}>
        <Text fontSize={13} color="#94a3b8">
          {notes.length > 0 ? `${notes.length} 条记录` : '暂无记录'}
        </Text>
      </ReAnimated.View>
